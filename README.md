...
- **AI Agent 断言** — LLM-as-judge 语义断言、文件断言（存在性/内容/JSON）、Session 与成本断言，适配 AI Agent 类应用测试
- **浏览器 E2E DSL** — 基于 Playwright 的声明式浏览器 action 与页面断言（无需手写脚本）
- **并行执行** — `ParallelSuiteExecutor` 支持多套件并发执行，可配置并发上限与隔离的变量上下文
- **重试与退避** — 用例级重试策略，支持线性/指数退避并记录每次 attempt
- **插件系统** — 通过 `e2e.yaml` 的 `plugins` 字段加载自定义断言类型与全局 setup/teardown
- **变量系统** — 支持 `{{config.*}}`、`{{env.*}}`、`{{runtime.*}}` 模板变量
- **韧性自愈** — 结构化错误码、预检健康检查、容器自动重启、端口冲突规避、孤儿资源清理、熔断器保护
- **测试持久化与趋势分析** — Drizzle ORM 持久化（SQLite / PostgreSQL / MySQL），Flaky Test 识别，通过率/时长趋势分析
...
│   │       ├── browser-executor.ts   # Playwright 浏览器 action 执行器
│   │       ├── parallel-engine.ts    # 多套件并行执行（并发上限 + 上下文隔离）
│   │       ├── retry-engine.ts       # 用例级重试与退避
│   │       ├── mock-generator.ts     # Mock 服务生成器（Fastify）
│   │       ├── assertion-engine.ts   # 断言 DSL 引擎（含文件断言、LLM-as-judge）
│   │       ├── assertion-plugin-registry.ts # 自定义断言插件注册表
│   │       ├── plugin-loader.ts      # e2e.yaml plugins[] 插件加载
│   │       ├── variable-resolver.ts  # 变量模板解析
│   │       ├── test-runner.ts        # RunnerRegistry
│   │       ├── reporter.ts           # Console/JSON/HTML 报告
│   │       ├── notifier.ts           # 可插拔通知（Console / Webhook / 自定义）
│   │       ├── runners/              # 测试运行器实现（yaml/vitest/pytest/shell/exec/playwright）
│   │       ├── agent-assertions/     # AI Agent 断言（session/cost；部分已晋升至 assertion-engine）
│   │       ├── resilience/           # 韧性与自愈子系统
...
│   │       ├── db/                   # Drizzle ORM 抽象层（规范查询路径）
│   │       │   ├── create-db.ts      #   Drizzle 实例工厂
│   │       │   ├── drizzle-history-store.ts   # 规范的历史查询层
│   │       │   ├── drizzle-knowledge-store.ts # 规范的知识库查询层
│   │       │   └── schema-{sqlite,pg,mysql}.ts # 多数据库 schema
│   │       ├── history/              # 测试持久化与趋势分析
│   │       │   ├── history-store.ts  #   SQLite 连接/迁移引导 + Store 工厂
...
├── specs/                # 规格驱动开发文档（spec/plan/tasks）
...
| CLI 框架 | Commander.js |
| 持久化存储 | Drizzle ORM（SQLite / PostgreSQL / MySQL；本地默认 SQLite WAL，底层连接由 better-sqlite3 引导） |
| 浏览器测试 | Playwright |
| Server / 同步 | Fastify（argusai-server）+ 队列化同步 |
